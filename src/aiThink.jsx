...
最后给出总体评分（1-10分）和最需要优先改进的1-2个问题。`,
...
请给出建设性、可落地的修改意见。每个建议都要有具体的改写示例。`,
      }
    case 'plot':
      return {
...
- 标注情绪混合方式（如：紧张中带着期待，愤怒中夹着心酸）`,
        system: `你是情感节奏设计师，擅长章节级情绪曲线与节拍设计。
...
6. 每个节拍要有"意图"：想让读者感受到什么？用了什么手法达成？`,
      }
    case 'translate': {
...
如果没发现问题，请说明角色表现一致，并指出哪些地方做得特别好（具体引用）。`,
...
4. 要关注细节：口头禅、语气词、断句习惯这些小地方往往最容易出问题`,
      }
    default:
      return null
  }
  ...
  try {
    ...
    try {
      ...
      if (outlines.length)
        parts.push(
          `【故事大纲】${outlines
            .map((o) => `${o.title}：${(o.content || '').slice(0, 150)}`)
            .join('\n')
            .slice(0, 6000)}`
        )
      if (timeline.length)
