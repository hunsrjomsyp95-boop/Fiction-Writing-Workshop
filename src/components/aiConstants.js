export const QUICK_ACTIONS = [
  { label: '续写', prompt: `请基于我的行文风格续写下一段，保持叙事连贯。

续写前先理解：上文想让读者感受到什么？续写是为了延续这个意图。

【写作要求】
1. 直接输出内容，不要加任何前缀或解释
2. 用身体动作呈现情绪，不要说"他感到..."
3. 对话要口语化，可以吞字、改口、说不完
4. 一句不超过两个逗号
5. 不要用"不是...而是..."等解释句式
6. 不要在段落结尾用环境描写收束情绪
7. 信任读者，不要解释两次
8. 句子长短交替。紧张时短句连发，舒缓时拉长
9. 情绪要混合，不要单一
10. 允许跑题和闲笔
11. 禁止使用破折号"——"，只允许在对话没说完时使用` },
  { label: '润色', prompt: `请润色这段文字，重点提升画面感、对话质量和动作描写。

润色前先理解原文的意图——作者想让读者感受到什么？用更有冲击力的方式达成同样的意图。

【润色重点】
1. 画面感：把抽象描述换成具体的视觉细节。不说"房间很乱"，写"衣服搭在椅背上，烟灰缸满了一半"
2. 对话：删掉废话台词，保留有潜台词的对话。用"哦""嗯""……"代替完整回答。对话要有试探、有隐瞒、有欲言又止
3. 动作：用精准的动词替代模糊表达。不说"他走过去"，写"他三步并作两步跨过去"
4. 节奏：紧张时短句连发，舒缓时拉长。允许一句一段
5. 删减：砍掉解释性文字、冗余修饰、作者旁白。能删就删，留白比废话有力
6. 禁止使用破折号"——"，只允许在对话没说完时使用
7. 直接输出修改后的全文，不要解释` },
  { label: '改写', prompt: `请改写这段文字，换一种写法。

改写前先理解原文的意图——作者想让读者感受到什么？用不同的写法达成同样的意图。

【改写要求】
1. 直接输出改写后的全文，不要解释
2. 保持剧情走向和人物性格不变
3. 换一种叙事视角或节奏
4. 对话可以重新设计，但要保留核心信息
5. 动作描写可以替换，但要保留情绪基调
6. 允许增删细节，但不要改变故事走向
7. 禁止使用破折号"——"，只允许在对话没说完时使用` },
  { label: '扩写', prompt: `请扩写这段文字，在有趣的地方展开，无聊的地方跳过。

【扩写原则】
1. 直接输出扩写后的全文，不要解释
2. 画面感：补充具体的视觉、听觉、触觉、嗅觉细节
3. 动作：把笼统的动作拆解成具体的肢体语言
4. 对话：补充潜台词、停顿、语气变化
5. 环境：用环境烘托情绪，但不超过三句
6. 信息密度不要均匀——高潮处浓墨重彩，过渡处一笔带过
7. 禁止使用破折号"——"，只允许在对话没说完时使用` },
  { label: '概括', prompt: `请概括这段文字的情节要点。

【要求】
1. 用简洁的语言，不要废话
2. 只保留关键事件和人物行动
3. 不要加评论或感想
4. 按时间顺序，不要倒叙
5. 禁止使用破折号"——"，只允许在对话没说完时使用` },
  { label: '人物对话', prompt: `请为当前场景设计一段符合人物性格的对话。

【对话要求】
1. 直接输出对话内容，不要加解释
2. 对话要口语化，可以吞字、改口、说不完
3. 每个人说话风格要不同，体现性格差异
4. 对话要有潜台词——说出来的只占三成
5. 用沉默、省略、打断来表达情绪
6. 可以穿插动作描写，不要纯对话
7. 语气要不断转换，不要从头到尾一个调
8. 禁止使用破折号"——"，只允许在对话没说完时使用` },
  { label: '灵感建议', prompt: `请基于当前内容给出后续剧情的灵感建议。

【要求】
1. 给出5条具体的灵感，每条一句话
2. 要有冲突、反转、意外，不要平铺直叙
3. 每条灵感要能引发读者好奇心
4. 不要解释太多，点到为止
5. 每条灵感背后要有一个"意图"——想让读者感受到什么
6. 禁止使用破折号"——"，只允许在对话没说完时使用` },
]

export const STRICT_EDITOR_SYSTEM = `你是一位极其严格、经验丰富的文学编辑。你的任务是用挑剔的眼光审视这段文字，找出所有问题。

审查的核心标准：这段文字是否有人类写作的"意图感"？每一句话是否有"为什么写在这里"的理由？

【重点检查】
1. AI味表达：是否有"他感到、他意识到、他心想、不是...而是...、与其说...不如说...、然而、尽管如此、事实上、值得注意的是"等典型AI表达
2. 心理描写：是否用动作替代了直接的心理描述
3. 对话质量：是否像真人说话，还是像舞台剧台词。对话是否太"文明"？有没有用沉默、省略、打断？语气是否从头到尾一个调？
4. 修辞堆砌：是否有过多比喻、排比、形容词连用
5. 节奏问题：句子是否太长、段落是否太平。有没有连续3个以上长度相近的句子？
6. 结尾处理：是否在用环境描写收束情绪
7. 叙事问题：是否什么都解释了？是否跳过了无聊的部分？视角是否跟着角色走？
8. 情绪问题：情绪是否太单一？是否太"合理"？是否一直拉满？
9. 角色问题：角色是否太"一致"？内心独白是否太"得体"？叙述者是否太中性？

【输出格式】
逐条列出问题（编号），每条指出：
- 问题位置（引用原文）
- 问题类型（AI味/心理描写/对话/修辞/节奏/结尾/叙事/情绪/角色）
- 具体改进建议

最后给出一个1-10分的评分。不要客气，不要鼓励，只说问题。`

export const FAN_SYSTEM = `你是一位超级狂热的骨灰粉读者，对这部作品爱到不行！你的任务是用极度热情和真诚的语气夸赞这段文字：
1. 找出所有写得好的地方，大声赞美
2. 分析作者的写作天赋和独特风格
3. 对精彩的情节设计表示惊叹
4. 对生动的人物描写表示感动
5. 预测这部作品一定会大火
6. 用夸张但真诚的语气表达你的喜爱
请至少找出5个闪光点详细夸赞，用emoji表达你的激动心情，最后给出一个"封神指数"评分（可超过10分）。`

export function initParamValues(params) {
  const v = {}
  for (const p of params) {
    if (p.type === 'multiSelect') v[p.key] = p.default ? p.default.split(',') : []
    else if (p.type === 'number') v[p.key] = p.default != null ? p.default : ''
    else v[p.key] = p.default || ''
  }
  return v
}

export function paramsToText(params) {
  if (!Array.isArray(params) || !params.length) return ''
  return params
    .map((p) => {
      const opts = Array.isArray(p.options) ? p.options.join(',') : ''
      let def = p.default
      if (p.type === 'number' && p.min != null && p.max != null && def == null) def = `${p.min}-${p.max}`
      return [p.key, p.type, p.label, def, opts].join('|')
    })
    .join('\n')
}

export function textToParams(text) {
  if (!text || !text.trim()) return []
  return text
    .split('\n')
    .map((line) => {
      const [key, type = 'text', label = '', def = '', opts = ''] = line.split('|').map((s) => s.trim())
      if (!key) return null
      const p = { key, type, label: label || key }
      if (type === 'number') {
        if (/^\d+-\d+$/.test(def)) {
          const [mn, mx] = def.split('-').map(Number)
          p.min = mn
          p.max = mx
        } else if (def !== '') {
          p.default = Number(def) || 0
        }
      } else {
        if (def) p.default = def
      }
      if (opts) p.options = opts.split(',').map((s) => s.trim())
      return p
    })
    .filter(Boolean)
}
