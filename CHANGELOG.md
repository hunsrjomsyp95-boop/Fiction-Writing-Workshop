...
### 新增
- **AI 整理合并设定**：世界观、人物、伏笔三类设定支持AI自动检测重复并合并
- **AI 自动配置人物关系**：在关系网页面点击"AI配置关系"，自动分析人物背景建立具体关系（父亲/母亲/师徒等）
- **AI 生成地图优化**：根据设定内容智能匹配图标和颜色，不再依赖名称关键词
...
### 新增
- 内置提示词新增5个模板：章节开头钩子、章节结尾悬念、对话润色、反派设计、力量体系设计
- 创建完整Skill知识库（D:\SkillKnowledgeBase\NovelWritingKnowledgeBase），包含12个写作指南模块：人物描写、角色设计、场景描写、开头写法、情感描写、文笔润色、对话写作、世界观构建、战斗场景、伏笔设计、章节大纲、实战练习
- **代码保护方案**：
  - JavaScript 代码混淆（javascript-obfuscator）：控制流平坦化、死代码注入、字符串加密
  - asar 完整性检测：启动时检测应用是否被篡改
  - 受保护的打包流程：`npm run dist:protected` 使用混淆后的代码打包
  - 新增 `scripts/obfuscate.js` 混淆脚本和 `electron/integrity.js` 完整性检测模块

### 优化
- seed.js内置提示词全面增强：角色生成、世界观生成、缩写摘要、细纲拆场景、灵感反推、文风学习等系统提示词从1行扩展为7-10条原则
- aiConstants.js快捷动作优化：概括、人物对话、灵感建议提示词增强，严格编辑系统扩展至12项检查维度
- aiThink.jsx思考动作优化：给出建议（含改写示例格式）、后续剧情、人物设计、情感节拍、角色一致性提示词深度优化
- ai.js核心AI函数优化：aiProofread增加6类检查、aiAssistant注入去AI味理念、EXTRACT_SYSTEM增加隐含信息推断、aiFilterContent增加价值判断
- ai-memory.js上下文系统优化：项目上下文字段扩充（世界观200字、人物增加外貌/关系、伏笔增加描述）
- 人物关系双向关系设定优化

### 修复
- 修复 electron/seed.js 第348行"对话润色"提示词对象缺少闭合 } 导致的JS语法错误

## [1.3.5] - 2026-08-21

### 优化
- AI提示词全面优化：润色专注画面感/对话/动作，改写换视角/节奏，扩写补充感官细节
- 所有提示词禁止使用破折号"——"，只允许在对话没说完时使用
- 选区润色提示词改进：从简单指令升级为专业润色原则

### 修复
- 修复AI流式输出报错 res.body.getReader is not a function（node-fetch v2兼容）
- 修复选区润色后替换位置错误（selection被提前清空）
- 修复选区润色润色全文而非选中内容
