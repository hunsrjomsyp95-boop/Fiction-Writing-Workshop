...
test('启动后显示首页', async () => {
  ...
  await window.waitForSelector('text=小说创作工坊', { timeout: 10000 })
  const body = await window.textContent('body')
  expect(body).toContain('小说创作工坊')
})

test('点击新建小说按钮弹出对话框', async () => {
  await window.waitForSelector('button:has-text("新建小说")', { timeout: 10000 })
...
test('输入小说名称后可以创建项目', async () => {
  await window.waitForSelector('input[placeholder*="星海"]', { timeout: 10000 })
  await window.locator('input[placeholder*="星海"]').fill('测试小说')
  await window.locator('.modal button.primary:has-text("创建")').click()
  await window.waitForSelector('.topbar .title', { timeout: 10000 })
...
test('工作区侧栏显示所有功能模块', async () => {
  ...
  await window.waitForSelector('.sidebar .nav-item .nav-label', { timeout: 10000 })
...
test('点击设置按钮打开设置页面', async () => {
  await closeModal()
  await window.waitForSelector('button[title="设置"]', { timeout: 10000 })
...
test('点击帮助按钮打开帮助页面', async () => {
  await closeModal()
  await window.waitForSelector('button[title="帮助"]', { timeout: 10000 })
...
test('返回首页按钮正常工作', async () => {
  await closeModal()
  await window.waitForSelector('button[title="返回首页"]', { timeout: 10000 })
